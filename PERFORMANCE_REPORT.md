...
| **BTree** | 245* | 25,676 | *永続性のために `sync_data()` を使用 |
...
| **BTree** | 40.79 ms* | 0.36 ms |

***LSM-tree の Get が Memory とほぼ同等** - Leveled Compaction と Index Block Cache の効果で、SSTable の検索効率が向上しています。

**BTree の特徴**:
- **Set**: Log エンジンとほぼ同等の性能 (245 vs 221 req/sec) - 各操作で `sync_data()` を実行
- **Get**: Memory に近い性能 (25,676 vs 26,726 req/sec) - B-Tree の O(log N) 検索
- **LSM-tree との比較**: Set は約 5x 遅い (グループコミットなし)、Get は同等レベル
...
3. **BTree vs LSM-tree**:
   - **書き込み**: LSM-tree が 5x 高速 (1,244 vs 245 req/sec)。LSM-tree はグループコミットで複数書き込みを1回の `sync_all()` でバッチ処理。
   - **読み取り**: ほぼ同等 (25,676 vs 26,468 req/sec)。
   - **理由**: BTree は各操作で `sync_data()` を実行するため、Log エンジンと同様の I/O ボトルネック。グループコミット実装により性能向上の余地あり。
...
**BTree Engine**: 新規実装したディスクベースB-Treeエンジンは、読み取り性能で Memory エンジンに近い性能を達成しました。書き込み性能は各操作で `sync_data()` を呼び出すため Log エンジンと同等です。グループコミット実装により LSM-tree と同等の書き込み性能が期待できます。
...
| ユースケース | 推奨エンジン | 理由 |
|-------------|-------------|------|
| 読み書きバランス | LSM-tree | グループコミットで書き込み高速 |
| 読み取り中心 | LSM-tree / BTree | 同等性能 |
| 書き込み中心 | LSM-tree | グループコミットで 5x 高速 |
| シンプルな永続化 | Log / BTree | 実装がシンプル |

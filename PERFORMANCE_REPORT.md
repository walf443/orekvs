...
| **BTree** | 1,117** | 24,108 | **グループコミット + `sync_data()` |
...
| **BTree** | 8.92 ms** | 0.39 ms |

***LSM-tree の Get が Memory とほぼ同等** - Leveled Compaction と Index Block Cache の効果で、SSTable の検索効率が向上しています。

**BTree の特徴**:
- **Set**: LSM-tree とほぼ同等の性能 (1,117 vs 1,244 req/sec) - グループコミットで複数書き込みをバッチ処理
- **Get**: Memory に近い性能 (24,108 vs 26,726 req/sec) - B-Tree の O(log N) 検索
...
|------|----------|------------------|--------|
| Set スループット | 245/sec | 1,117/sec | **4.5x** |
| Set レイテンシ | 40.79 ms | 8.92 ms | **4.6x** |
...
3. **BTree vs LSM-tree**:
   - **書き込み**: ほぼ同等 (1,117 vs 1,244 req/sec)。両エンジンともグループコミットで複数書き込みをバッチ処理。
   - **読み取り**: ほぼ同等 (24,108 vs 26,468 req/sec)。
...
**BTree Engine**: 新規実装したディスクベースB-Treeエンジンは、グループコミットにより LSM-tree と同等の読み書き性能を達成しました。Set: 1,117 req/sec、Get: 24,108 req/sec で、永続性を維持しながら高い性能を実現しています。

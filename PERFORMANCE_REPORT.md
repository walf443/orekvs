...
| **LSM-tree** | 267 | 37.45 ms | 0.22x | cas_lock によるシリアライズ |
| **BTree** | 266 | 37.59 ms | 0.25x | cas_lock によるシリアライズ |

**CAS 性能の特徴**:
- **Memory**: CAS は SET とほぼ同等。永続化オーバーヘッドがなく、SkipMap の CAS 操作で実装。
- **Log**: CAS が SET より高速。書き込みが元々シリアライズされているため、CAS ロックがボトルネックにならない。
- **LSM-tree / BTree**: CAS は SET の約 1/4〜1/5。
  - SET は `cas_lock` を取得しないため、グループコミットで並列化される
  - CAS は `cas_lock` (Mutex) でシリアライズされるため、並列度が効かない
  - 単一スレッド (p=1) では SET と CAS はほぼ同等 (~230 req/sec)

...
impl LogEngine {
    pub fn new(file_path: String, compaction_threshold: u64) -> Self {
        ...
        if current_len > HEADER_SIZE {
            file.seek(SeekFrom::Start(HEADER_SIZE)).expect("Failed to seek to data");
            ...
            loop {
                ...
                file.read_exact(&mut klen_bytes).expect("Failed to read key length");
                let mut vlen_bytes = [0u8; 8];
                file.read_exact(&mut vlen_bytes).expect("Failed to read value length");
                ...
                let value_offset = file.stream_position().expect("Failed to get stream position");
                ...
                if val_len == u64::MAX {
                    ...
                    // Skip value
                    file.seek(SeekFrom::Current(val_len as i64)).expect("Failed to skip value");
    ...
    fn compact(&self) -> Result<(), Status> {
        // Set compacting flag (redundant if called from set, but safe)
        ...
        for (key, &(offset, length)) in index.iter() {
            ...
            if let Err(e) = new_file.write_all(&timestamp.to_le_bytes()) { self.is_compacting.store(false, Ordering::SeqCst); return Err(Status::internal(e.to_string())); }
            if let Err(e) = new_file.write_all(&key_len.to_le_bytes()) { self.is_compacting.store(false, Ordering::SeqCst); return Err(Status::internal(e.to_string())); }
            if let Err(e) = new_file.write_all(&val_len.to_le_bytes()) { self.is_compacting.store(false, Ordering::SeqCst); return Err(Status::internal(e.to_string())); }
            if let Err(e) = new_file.write_all(key_bytes) { self.is_compacting.store(false, Ordering::SeqCst); return Err(Status::internal(e.to_string())); }
            if let Err(e) = new_file.write_all(&value) { self.is_compacting.store(false, Ordering::SeqCst); return Err(Status::internal(e.to_string())); }

            let current_pos = match new_file.stream_position() {
                Ok(pos) => pos,
                Err(e) => { self.is_compacting.store(false, Ordering::SeqCst); return Err(Status::internal(e.to_string())); }
            };
...
impl Engine for LogEngine {
    fn set(&self, key: String, value: String) -> Result<(), Status> {
        ...
        let should_compact = {
            ...
            writer.metadata().map_err(|e| Status::internal(e.to_string()))?.len() > self.compaction_threshold
        };

        if should_compact {
            // Check if compaction is already running
            // If compare_exchange returns Ok(_), it means the value was false and we set it to true.
            if self.is_compacting.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst).is_ok() {
    ...
    fn delete(&self, key: String) -> Result<(), Status> {
        ...
        // Check size: timestamp(8) + key_len(8) + val_len(8) + key
        ...
        let should_compact = {
            ...
            // Get current offset
            writer.seek(SeekFrom::End(0)).map_err(|e| Status::internal(e.to_string()))?;

            // Write Format: [timestamp][key_len][val_len(MAX)][key][(no value)]
            writer.write_all(&timestamp.to_le_bytes()).map_err(|e| Status::internal(e.to_string()))?;
            writer.write_all(&key_len.to_le_bytes()).map_err(|e| Status::internal(e.to_string()))?;
            writer.write_all(&val_len.to_le_bytes()).map_err(|e| Status::internal(e.to_string()))?;
            writer.write_all(key_bytes).map_err(|e| Status::internal(e.to_string()))?;
            // No value data written for tombstone
...
mod tests {
    ...
    async fn test_compaction_runs_async() {
        ...
        // 閾値を150バイトに設定
        // 各エントリはおよそ: 8(ts) + 8(klen) + 8(vlen) + 4(key) + 4(val) = 32 bytes + overhead
        // 正確には: 24 (header) + 4 ("keyX") + 4 ("valX") = 32 bytes
        // 3回書き込むと 32 * 3 = 96 bytes... あれ、もっと小さいですね。
        // ヘッダー(10bytes) + 96 = 106 bytes
        // 余裕を持って閾値を 80バイトにします。
        
        let engine = LogEngine::new(file_path.clone(), 80);
        
        // 1. Write key1 (size ~32 bytes, total file ~42 bytes)
        engine.set("key1".to_string(), "val1".to_string()).unwrap();
        
        // 2. Write key2 (size ~32 bytes, total file ~74 bytes)
        engine.set("key2".to_string(), "val2".to_string()).unwrap();
        
        // Check size before compaction trigger
        let size_before = std::fs::metadata(&file_path).unwrap().len();
        assert!(size_before < 80, "File size should be small: {}", size_before);

        // 3. Update key1 (size ~36 bytes, total file ~110 bytes > 80) -> Trigger Compaction
        engine.set("key1".to_string(), "new_val1".to_string()).unwrap();
        
        // Wait for async compaction to finish
        tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
        
        // Check size after compaction
        // Expected: key1(new_val1) + key2(val2) + Header
        // 36 + 32 + 10 = 78 bytes
        let size_after = std::fs::metadata(&file_path).unwrap().len();
        
        // 元のサイズ(110)より小さくなっているはず
        assert!(size_after < 110, "Compaction should reduce file size. Current: {}", size_after);
    ...
    fn test_delete() {
        ...
        {
            let engine = LogEngine::new(file_path.clone(), 1024);

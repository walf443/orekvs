...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &cli.command {
        ...
        Commands::Test { addr, command } => match command {
            ...
            TestCommands::RandomCas {
                count,
                parallel,
                key_range,
                value_size,
            } => run_test_cas(addr.clone(), *count, *parallel, *key_range, *value_size).await,
...
async fn run_test_cas(
    ...
    let mut key_values: std::collections::HashMap<String, String> =
    ...
    let key_values = Arc::new(std::sync::RwLock::new(key_values));
    let mut handles = Vec::with_capacity(count);
    let start_time = Instant::now();

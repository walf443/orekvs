...
- **日付**: 2026-01-29 (count_prefix_keys_mmap 最適化後)

### 結果

| エンジン | プレフィックス | マッチ数 | Time (ms) | ops/sec |
|----------|----------------|----------|-----------|---------|
| **Memory** | log: (20%) | 2,000 | 12.1 | 8,292 |
| **Memory** | log:001 (1%) | 100 | 10.4 | 9,592 |
| **Memory** | log:0010 (0.1%) | 10 | 5.1 | 19,777 |
| **Memory** | nonexistent: | 0 | 5.4 | 18,408 |
| **Log** | log: (20%) | 2,000 | 6.6 | 15,079 |
| **Log** | log:001 (1%) | 100 | 7.0 | 14,351 |
| **Log** | log:0010 (0.1%) | 10 | 3.2 | 30,863 |
| **Log** | nonexistent: | 0 | 3.8 | 26,301 |
| **BTree** | log: (20%) | 2,000 | 16.5 | 6,067 |
| **BTree** | log:001 (1%) | 100 | 2.1 | 47,087 |
| **BTree** | log:0010 (0.1%) | 10 | 1.2 | 83,036 |
| **BTree** | nonexistent: | 0 | 1.0 | 100,125 |
| **LSM-tree** | log: (20%) | 2,000 | 30.1 | 3,319 |
| **LSM-tree** | log:001 (1%) | 100 | 0.95 | **105,485** |
| **LSM-tree** | log:0010 (0.1%) | 10 | 0.12 | **824,171** |
| **LSM-tree** | nonexistent: | 0 | 0.02 | **4,347,826** |
...
4. **count_prefix_keys_mmap**: Vec確保を回避し、直接カウントを返す専用関数
   - seen_keysをmutable参照で受け取り、複数SSTable間の重複排除
   - 中間タプル生成を回避してインラインでカウント
   - 低〜中マッチ率で30-40%の性能向上

**注釈**:
- **LSM-tree** がマッチ率が低い場合に最も高速: 範囲スキャンにより O(log n + k) で検索（0%マッチで **4,347,826 ops/sec**）
- **BTree** も効率的: リーフノード走査で O(log n + k)
- **Memory/Log** は全キーを走査するため、マッチ率に関わらずほぼ一定のオーバーヘッド
- **高マッチ率** (20%) では Log が最速: LSM-tree は重複排除のための HashSet 操作がボトルネック
...
| Count (低マッチ率) | LSM-tree | 範囲スキャン + Key Range Filter で最高速 |
| Count (高マッチ率) | Log | シンプルなインメモリインデックスで効率的 |
